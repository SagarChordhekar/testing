...
getApiDetails(serviceName,clientCode): Promise<any> {
    console.log("In API Call Service Name ",serviceName);
    console.log("In API Call Client Code ",clientCode);
    return new Promise((resolve, reject) => {
...
postUAT(data):Promise<any>{
    ...
    return new Promise ((resolve,reject)=>{
        this.http.post(baseUrl +"project/initiateUAT",data).pipe(map(Response=>Response))
        .subscribe((response:any)=>{
            resolve(response);
        },reject);
        })
